...
BEGIN
    ...
    UPDATE players p
        JOIN (SELECT player_id,
              ...
              GROUP BY player_id) ranked_bets ON p.player_id = ranked_bets.player_id
    SET p.account_balance = CASE
                                WHEN ranked_bets.player_rank = 1
                                    THEN p.account_balance + (active_tournament_prize_pool * 0.5)
                                WHEN ranked_bets.player_rank = 2
                                    THEN p.account_balance + (active_tournament_prize_pool * 0.3)
                                WHEN ranked_bets.player_rank = 3
                                    THEN p.account_balance + (active_tournament_prize_pool * 0.2)
